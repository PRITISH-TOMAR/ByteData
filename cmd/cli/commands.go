package cli

import (
	"errors"
	"fmt"
	"strings"
	"github.com/PRITISH-TOMAR/byted/internal/kv"
)

func ExecuteCommand(input string, engine *kv.KVEngine) error {
	input = strings.TrimSpace(input)
	if input == "" {
		return nil
	}
	
	parts := strings.Fields(input)
	command := parts[0]

	switch command {
	case "put":
		return handlePut(parts, engine)
	case "get":
		return handleGet(parts, engine)
	case "del", "delete":
		return handleDelete(parts, engine)
	case "range":
		return handleRange(parts, engine)
	case "exit", "quit":
		return handleExit()
	case "help":
		return handleHelp(1)
	default:
		return handleUnknown(command)
	}
}

func handlePut(parts []string, engine *kv.KVEngine) error {
	if len(parts) < 3 {
		return errors.New("usage: put <key> <value>")
	}
	
	key := parts[1]
	value := strings.Join(parts[2:], " ")

	lsn, err := engine.Put([]byte(key), []byte(value))
	if err != nil {
		return fmt.Errorf("put failed: %v", err)
	}
	
	fmt.Printf("Put successful. LSN: %d\n", lsn)
	return nil
}

func handleGet(parts []string, engine *kv.KVEngine) error {
	if len(parts) != 2 {
		return errors.New("usage: get <key>")
	}
	
	key := parts[1]
	value, err := engine.Get([]byte(key))
	if err != nil {
		return fmt.Errorf("get failed: %v", err)
	}
	
	if value == nil {
		fmt.Println("Key not found")
	} else {
		fmt.Printf("Value: %s\n", string(value))
	}
	
	return nil
}

func handleDelete(parts []string, engine *kv.KVEngine) error {
	if len(parts) != 2 {
		return errors.New("usage: del <key>")
	}
	
	key := parts[1]
	lsn, err := engine.Delete([]byte(key))
	if err != nil {
		return fmt.Errorf("delete failed: %v", err)
	}
	
	fmt.Printf("Delete successful. LSN: %d\n", lsn)
	return nil
}


func handleRange(parts []string, engine *kv.KVEngine) error {
	if len(parts) != 3 {
		return errors.New("usage: range <startKey> <endKey>")
	}
	
	startKey := parts[1]
	endKey := parts[2]
	results := engine.Range([]byte(startKey), []byte(endKey))
	
	if len(results) == 0 {
		fmt.Println("No keys found in the specified range")
	} else {
		fmt.Printf("Found %d key(s):\n", len(results))
		for _, v := range results {
			fmt.Printf("  Key: %s, Value: %s\n", v.Key, v.Value)
		}
	}
	
	return nil
}




func handleUnknown(command string) error {
	return fmt.Errorf("unknown command: '%s'. Type 'help' for available commands", command)
}

func printHelpBucket() {
	fmt.Println("Available commands:")
		fmt.Println("  put <key> <value>    - Add or update a key-value pair")
		fmt.Println("  get <key>            - Retrieve the value for a given key")
		fmt.Println("  del <key>            - Delete a key-value pair")
		fmt.Println("  range <start> <end>  - Retrieve all key-value pairs in the specified key range")
		fmt.Println("  exit                 - Exit the CLI")
		fmt.Println("  help                 - Show this help message")
}
